	...
	****/
	function _endTagProcessor($__parser, $__tag_name)
	{
		...
		if(strtolower($__tag_name) == 'externalpage')
		{
			...
			// $query .= '(externalpage, title, description, ages, mediadate, priority, topic)';
			// $query .= ' VALUES("' . addslashes($this->external_page) . '", "' . addslashes(trim($this->title)) . '", "' . addslashes(trim($this->description)) . '", "' . addslashes(trim($this->ages)) . '", "' . addslashes($this->mediadate) . '", "' . addslashes($this->priority) .'", "' . addslashes($this->topic) .'")';
			// if ( stristr($this->topic,"Top/World/Chinese_Simplified") )
			if ( stristr($this->topic,"Top/World/Chinese_Simplified") )
			{
				$query = addslashes(trim($this->title)) . "\t" . str_ireplace("Top/World/Chinese_Simplified","",$this->topic) ."\t" . addslashes($this->external_page) . "\n";
